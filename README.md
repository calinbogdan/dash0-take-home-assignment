...
[`queue.ts`](nodejs/src/queue.ts) declares the schema once: `LogEntrySchema`. The TypeScript `LogEntry` type is `z.infer<typeof LogEntrySchema>`, so the runtime schema and the static type can never drift.
...
- Failures are transient. Retrying the same entry up to three times is the right policy; a DLQ is out of scope.
